...
import { Hotel, HotelRoom } from '../../src/types';
...
function randomRooms(): HotelRoom[] {
    ...
    for (let i = 0; i < count; i++) {
        rooms.push({
            ...
            images: [
                `https://images.unsplash.com/photo-${Math.floor(Math.random() * 1000000000000)}?auto=format&fit=crop&w=800&q=80`,
                `https://images.unsplash.com/photo-${Math.floor(Math.random() * 1000000000000)}?auto=format&fit=crop&w=800&q=80`
            ],
...
export const hotels: Hotel[] = Array.from({ length: 50 }, (_, i) => {
    ...
    return {
        ...
        amenities: randomAmenities(),
        images: [
            `https://images.unsplash.com/${unsplashImages[Math.floor(Math.random() * unsplashImages.length)]}?auto=format&fit=crop&w=800&q=80`,
            `https://images.unsplash.com/${unsplashImages[Math.floor(Math.random() * unsplashImages.length)]}?auto=format&fit=crop&w=800&q=80`
        ],
        rooms,
        contact: {
